import {etInfo, tongji} from "./request";
import {getLayerParameter} from "./layers";

class ZhengSanFaUtils {
    ...
    init() {
        ...
        mapApis.updateMagmMethod({
            "POLYGON"(magm,obj,geojson) {
                $this$._tmpFn = () => {
                    let dd = $this.eday * 8 - 7
                    if (dd < 10) {
                        dd = "00" + dd;
                    } else if (dd < 100) {
                        dd = "0" + dd;
                    }
                    tongji(`et:a_${$this.year}${dd}`,JSON.stringify(geojson.toGeoJson()))
                        .then(_ => {
                            console.log(_);
                            let a = [];
                            for (let i in _) {
                        ...
                        $this$.setOption(datas,
                            etInfo.kindName[$this.type.toLowerCase()],
                            `${$this.year}年第${$this.eday}旬`);
                        $this$.updateMapEchart(datas,etInfo.kindName,geojson);
                    });
                };
                $this$._tmpFn();
                // utils.setOption();
    ...
    updateLayer() {
        ...
        mapApis.addTimelineLayer(
            $this.year + '-1-1',
            $this.year + "-12-31",
            window.allUrls.geoserver + "et/wms",
            ...
            ,function ({year,month,day,dd}) {
                ...
                return {
                    layers: `et:a_${year}${dd}`,
                    query_layers: `et:a_${year}${dd}`,
