...
import { requestFscsByNameYears,requestPjsslByName } from "./requests";
import { _STATION } from "./datas";

class XianHaiYanChenJianCeUtils {
    /**
     *
    ...
    ParsingPjsslData(val) {
        ...
        if (val) {
            for (var key in val) {
                if (key !== "name") {
                    ...
                    if (!legendData.includes(key)) legendData.push(key);
                }
            }
        }
        //排序
        ...
        optcls.setAxis([]);
        optcls.setDataset(dataList, ["方向", "输送量"], ["name", "value"]);
        optcls.setSeries({
            type: "pie", name: "沙尘输送量", encode: {itemName: 0, value: 1}, right: 50
            // roseType: 'area'
        });
        optcls.setLegend({
            type: 'scroll',
    ...
    // 创建地图中的点及图表
    createEchart() {
        _STATION.forEach(point => {
            let obj = window.mapApis.createEchartDom(
                    this.eAndc.entity,
                    this.eAndc.css3Renderer,
                    { title: point.title,long: +point.long, lat: +point.lat }
                );
            // obj.echartElement.style.display = 'block';
            this.pointEchart[point.title] = obj;
        });
    }
