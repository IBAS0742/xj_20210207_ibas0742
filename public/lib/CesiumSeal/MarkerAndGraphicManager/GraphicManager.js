const GraphicManager = (function () {
    ...
    return class {
         ...
         */
        constructor(viewer, options = {},offset = {x:0,y:0}) {
            ...
            */
            ...
            this.positions = []
            this.tip = new CursorTip('',null,null,offset);
        ...
        importByEntity(ent,type) {
            let id = this.generateId();
            let graphic = null;
            if (type === 'line') {
                graphic = new CesiumPolyline(viewer,ent);
            } else {
                graphic = new CesiumPolygon(viewer,ent);
            }
            if (graphic) {
                graphic.gvid = id;
                graphic.gvname = `name_${id}`
                this.manager.set(id, graphic)
                const evt = new CustomEvent('addEvent', {
                    detail: {
                        gvid: graphic.gvid,
        ...
        // type = line,polygon
        importByEntities(entities,type) {
            entities.forEach(ent => {
                this.importByEntity(ent,type);
            });
        }
